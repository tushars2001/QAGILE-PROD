    ...
    if request.method == 'POST':
        ...
        if 'name' in request.POST and 'file_type' in request.POST:
            ...
            if response.status_code == 201:
                return redirect("/create-structure/?message=Definition Created&name=" + request.POST['name'])
            else:
...
def add_row(request):
    if request.method != 'POST':
        return redirect('/create-structure/?' + request.META['QUERY_STRING'])
...
def create_copy(request):
    ...
    if request.method == "POST" and 'name' in request.POST:
        copy = xapi.create_copy(request)
        context_data = {"data": copy}
        return redirect("/view-definition/?data=" + copy)

    return redirect("/view-definition/?error=Invalid Request")
