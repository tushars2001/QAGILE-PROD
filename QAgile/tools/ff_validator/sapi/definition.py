...
def create_copy(account_name, name):
    ...
    sql_definition = """
            insert into `tools`.`definition` (name, description, file_type, is_header, account_name) 
            SELECT '""" + new_name + """' as name, description, file_type, is_header, account_name FROM tools.definition 
            where name='""" + name + """'
            """

    sql_structure = """
                insert into `tools`.`structure` (definition_name, `order`, col_name, col_data_type, required, 
                max_len, is_fixed_len, format, min_value, max_value, decimals, all_null, is_pk, account_name) 
