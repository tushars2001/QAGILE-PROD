...
def get_all_person():
    with connection.cursor() as cursor:
        sql = """SELECT p.*, l.city, cast(IFNULL(r.rate,0) as char) as rate, IFNULL(ro.role,'') as role, 
...
def update_person(postdata):
    ...
    fields = {'first_name': first_name, 'last_name': last_name, 'email': email, 'category': category,
                ...
                'vnid': vnid, 'rate_id': rate_id, 'domains': domains}
    ...
    if not exist:
        ...
        sql = """update qagile_person 
                    set vnid = %(vnid)s , 
                        ...
                        phone = %(phone)s ,
                        role_id = %(role_id)s , 
                        location_id = %(location_id)s, rate_id = %(rate_id)s
                    where vnid = %(vnid)s
                """
    print(sql)
